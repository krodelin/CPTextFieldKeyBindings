...
var ENV = require("system").env,
    ...
    OS = require("os"),
    projectName = "CPTextFieldKeyBindings";
...
app (projectName, function(task)
{
    ...
    task.setBuildIntermediatesPath(FILE.join("Build", "CPTextFieldKeyBindings.build", configuration));
    task.setBuildPath(FILE.join("Build", configuration));

    task.setProductName("CPTextFieldKeyBindings");
    task.setIdentifier("com.krodelin.CPTextFieldKeyBindings");
    task.setVersion("1.0");
    task.setAuthor("Krodelin Software Solutions");
    task.setEmail("info@krodelin.com");
    task.setSummary("CPTextFieldKeyBindings");
...
task ("desktop", ["release"], function()
{
    ...
    require("cappuccino/nativehost").buildNativeHost(FILE.join("Build", "Release", projectName), FILE.join("Build", "Desktop", projectName, "CPTextFieldKeyBindings.app"));
    printResults("Desktop")
});

task ("run-desktop", ["desktop"], function()
{
    OS.system([FILE.join("Build", "Desktop", projectName, "CPTextFieldKeyBindings.app", "Contents", "MacOS", "NativeHost"), "-i"]);
});
